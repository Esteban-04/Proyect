...
// Icono de caja exacto al de la imagen
const BoxIcon = (props: React.SVGProps<SVGSVGElement>) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-14v4m0 0l8 4m-8-4l-8 4m0-14v10l8 4" />
    <path strokeLinecap="round" strokeLinejoin="round" d="M12 12v7" />
  </svg>
);

// Icono de archivo/caja alternativo más simple (como el de la imagen)
const CrateIcon = (props: React.SVGProps<SVGSVGElement>) => (
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ users, setUsers, onContinue, onLogout, currentUser }) => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col h-[95vh] sm:h-[90vh] relative font-sans">
      ...
      {/* HEADER REFINADO - DISEÑO IGUAL A LA IMAGEN */}
      <div className="bg-[#0b1626] px-6 sm:px-10 py-10 flex flex-col md:flex-row justify-between items-center gap-6 shrink-0 border-b border-white/5">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 w-full md:w-auto">
            {/* Cápsula de idioma estilo imagen - Botón activo blanco */}
            ...
            {/* Botón Guardar - VERDE VIVO (#00e676) con icono de caja */}
            ...
            {/* Botón Agregar - AZUL VIVO (#2979ff) */}
            ...
            {/* Botón Cerrar Sesión - ROJO VIVO (#ff1744) */}
      ...
      <div className="flex-grow overflow-y-auto bg-[#f8fafc] p-6 sm:p-10">
          {activeTab === 'users' ? (
              <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-x-auto shadow-sm p-4">
                                    ...
                                    <tr key={user.username} className="hover:bg-slate-50/40 transition-colors group">
                                        <td className="px-8 py-8">
                                            <div className="flex items-center gap-4">
                                                <button onClick={() => toggleUserStatus(user.username)} disabled={isSuper} className={`relative h-6 w-11 rounded-full transition-colors ${!user.isDisabled ? 'bg-[#00e676]' : 'bg-slate-200'} disabled:opacity-30 cursor-pointer`}><span className={`absolute left-1 top-1 inline-block h-4 w-4 transform bg-white rounded-full transition-transform ${!user.isDisabled ? 'translate-x-5' : 'translate-x-0'}`} /></button>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col animate-in zoom-in duration-300">
                  {showAddUserModal && (
                      <form onSubmit={handleAddUser} className="flex flex-col h-full">
                          ...
                          <div className="p-10 bg-slate-50 border-t flex gap-4"><button type="button" onClick={() => setShowAddUserModal(false)} className="flex-1 py-5 font-black text-slate-400 uppercase tracking-widest hover:text-slate-600">{t('cancelButton')}</button><button type="submit" className="flex-1 py-5 bg-[#2979ff] text-white rounded-2xl font-black uppercase tracking-widest shadow-2xl hover:bg-blue-700 transition-all">{t('createButton')}</button></div>
                      </form>
                  )}
                  ...
                  {showEditUserModal && editUserData && (
                      <form onSubmit={handleUpdateUser} className="p-12 space-y-8">
                          ...
                          <div className="flex justify-end items-center gap-8 mt-12"><button type="button" onClick={() => setShowEditUserModal(false)} className="text-slate-400 font-black uppercase tracking-widest hover:text-slate-600">{t('cancelButton')}</button><button type="submit" className="bg-[#2979ff] text-white px-12 py-5 rounded-2xl font-black uppercase tracking-widest shadow-2xl hover:bg-blue-700 active:scale-95 transition-all">{t('updateButton')}</button></div>
                      </form>
                  )}
              </div>
          </div>
